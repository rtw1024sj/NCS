			...
			for (int i = 0; i < CommonMember.count; i++) {
				if (cmbr_arr[i] instanceof Gujikja) {
					sb.append(cmbr_arr[i].getName() +"\t"
						+ ((Gujikja)cmbr_arr[i]).getGender() +"\t"
						+ ((Gujikja)cmbr_arr[i]).getAge() +"\n");
				}
			}// end of for--------------
	...
	private void view_all_gujikja_gender(Scanner sc, CommonMember[] cmbr_arr) {
		...
		if (cnt == 0) { // 구직자가 없는 경우
			...
			for (int i = 0; i < CommonMember.count; i++) {
				 ...
				 if( cmbr_arr[i] instanceof Gujikja && input_gender.equals(((Gujikja)cmbr_arr[i]).getGender() )) {
					 isSearch = true;
					 sb.append( cmbr_arr[i].getName() + "\t"
					 		  + ((Gujikja)cmbr_arr[i]).getGender() + "\n");
				 }
		...
		if(cnt == 0) { // 구직자가 없는 경우
			...
			if(!is_ageline_search) { // 검색하고자 하는 연령대에 해당하는 구직자가 존재하지 않는 경우 
				...
				for(int i=0; i<CommonMember.count; i++) {
					 ...
					 if( cmbr_arr[i] instanceof Gujikja && Integer.parseInt(str_ageLine) == ((Gujikja)cmbr_arr[i]).getAge()/10*10) {
