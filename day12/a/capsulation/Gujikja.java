...
*/
	...
	// // == getter, setter 하기 == // 
	public void setUserid(String userid) {	// 값을 넣을 땐 set, 읽어올 때는 get
		...
		if (userid == null || userid.isBlank()) {
			...
			// 그리고 userid 의 길이는 최소 3글자, 최대 10 글자로 제한한다.
			...
			Matcher m = p.matcher(userid);	// userid 를 p 패턴에 맞춰준 값을 m 값에 맞춰준다/
			
			// == 3. 판별하도록 한다. == //
			if( m.matches() ) {	// matches 는 boolean 타입이다. 일치하면 true, 일치하지 않으면 false
				this.userid = userid;	//true 일 경우 ( id 가 p 팬턴에 맞을경우)
			}
			else {
				System.out.println("[경고] 아이디는 첫글자는 영문이어야 하고 나머지는 "
						+ "영문 또는 숫자로만 이루어진 글자길이는 3~10글자만 가능합니다.\n");
...
*/
	...
	public void setName(String name) {
		...
		// == 2. 문자열이 주어진 정규식 패턴과 일치하는지 판별하는 객체를 생성한다. == //
		Matcher m = p.matcher(name);	// userid 를 p 패턴에 맞춰준 값을 m 값에 맞춰준다/
		
		// == 3. 판별하도록 한다. == //
		if( m.matches() ) {	// matches 는 boolean 타입이다. 일치하면 true, 일치하지 않으면 false
			this.name = name;	//true 일 경우 ( id 가 p 팬턴에 맞을경우)
		}
		else {
			System.out.println("[경고] 성명은 공백이 없는 한글로만 이루어져야 하며 최소 2글자 이상 최대 6글자로만 되어져야 합니다.\n");
