...
*/
			...
			switch (operator) { // 비교 대상과 operator 는 String 타입.(키보드에 입력한 값이 같은지 묻는다)
				case "+": // operator 값이 "+" 와 같다라면
					calc_result = num1 + num2;
					break; // switch 문장에서 break; 를 만나면 switch (operator){ } 부분을 빠져나가라는 말이다.  
...
*/
			switch (operator) {
			case "/":
				// result 에 나머지가 없을 경우 calc_result 를 int 형으로 출력
				if(num1 % num1 == 0) {
					result = num1 + operator + num2 + "=" + (int) calc_result;	
				} else {	// 나머지가 있을 경우 double 로 출력
				result = num1 + operator + num2 + "=" + calc_result;
			...
			default:	// + - * 일 경우 calc_result 를 int 형으로 출력
				result = num1 + operator + num2 + "=" + (int) calc_result;
				break;
			} // end of switch (operator)
			// break; 가 나오면 switch 문을 빠져나온다.
