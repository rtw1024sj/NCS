...
public class Ctrl_gujikja extends Ctrl_common {
	...
	// == 연령대 검색해주는 메소드 == //
	void search_ageLine(Scanner sc, Gujikja_imple[] gu_arr) {
	...
	// == 연령대 및 성별 검색해주는 메소드 == //
	void search_ageLine_gender(Scanner sc, Gujikja_imple[] gu_arr) {
	...
	*/
			...
			try {
				...
				for (int i = 0; i < Recruit.count; i++) {
					...
					if (!date_finish_day.before(today) ) {
						...
						sb.append(rc_arr[i].getRecruit_no() + "\t\t" + rc_arr[i].getCp().getName() + "\t"
								+ rc_arr[i].getCp().getJob_type() + "\t"
								+ new DecimalFormat("#,###").format(rc_arr[i].getCp().getSeed_money()) + "만원\t"
								+ rc_arr[i].getWork_type() + "\t" + rc_arr[i].getCnt() + "\t"
								+ rc_arr[i].getRegister_day().substring(0, 4) + "-"
								+ rc_arr[i].getRegister_day().substring(4, 6) + "-"
								+ rc_arr[i].getRegister_day().substring(6) + "\t"
								+ rc_arr[i].getFinish_day().substring(0, 4) + "-"
	...
	private void input_recruitApply(Scanner sc, Gujikja_imple login_gu, Recruit[] rc_arr, RecruitApply[] rcApply_arr) {
		...
		if (str_my_recruit_no.length() > 0) {
			...
			if (str_my_recruit_no_arr.length == Recruit.count) {
		...
		do {
			...
			for (int i = 0; i < Recruit.count; i++) {
				...
				if (input_recruit_no.equals(String.valueOf(rc_arr[i].getRecruit_no()))) {
					is_existence = true;
					rc = rc_arr[i]; // 해당 채용 공고를 rc 에 넣어둔다
					break; // for 문 break;
				}
			} // end of for------------------------
